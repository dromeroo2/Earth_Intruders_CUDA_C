...
//DEFINIMOS EL TAMAÑO DE NUESTAR TESELA
#define TILE 3
...
__global__ void descensoYDesintegracion(char* dev_matriz, curandState* state, short* d_filas, short* d_columnas) {
    short indice = threadIdx.x;  // aquí almaceno el índice del hilo dentro del bloque 
    short filaActual = blockIdx.x; //
    ...
    if (filaActual < filas - 1) {
        if (dev_matriz[filaActual * columnas + indice] == letraA ||
            dev_matriz[filaActual * columnas + indice] == letraN ||
            dev_matriz[filaActual * columnas + indice] == hueco || // AÑADIDO NUEVO
            dev_matriz[filaActual * columnas + indice] == letraC ||
            dev_matriz[filaActual * columnas + indice] == letraD ||
            dev_matriz[filaActual * columnas + indice] == letraR ||
            dev_matriz[filaActual * columnas + indice] == letraX) {

            if (dev_matriz[filaSiguiente * columnas + indice] == letraBloque &&
                (dev_matriz[filaActual * columnas + indice] == letraA ||
                    dev_matriz[filaActual * columnas + indice] == hueco || // AÑADIDO NUEVO
                    dev_matriz[filaActual * columnas + indice] == letraN ||
                    dev_matriz[filaActual * columnas + indice] == letraC ||
                    dev_matriz[filaActual * columnas + indice] == letraD)) {
                //dev_matriz[filaActual * columnas + indice] = hueco;

                if (dev_matriz[filaActual * columnas + indice] == letraD) {
                    //Explosión de radio 5
                    const short radio = 5;
                    ...
                    for (short i = -radio; i <= radio; ++i) {
                        for (short k = -radio; k <= radio; ++k) {
                            ...
                            short nuevaColumna = indice + k;
                            // Verifico si las nuevas coordenadas están dentro de los límites de la matriz:
                            if (nuevaFila >= 0 && nuevaFila < filas && nuevaColumna >= 0 && nuevaColumna < columnas) {
                                // Asigno el valor hueco a la celda en las nuevas coordenadas si es distinta de un muro:
                ...
                dev_matriz[filaSiguiente * columnas + indice] = letraBloque;//dev_matriz[filaActual * columnas + indice];
            }
            else if (dev_matriz[filaSiguiente * columnas + indice] == letraBloque &&
                (dev_matriz[filaActual * columnas + indice] == letraR ||
                    dev_matriz[filaActual * columnas + indice] == letraX)) {

                dev_matriz[filaSiguiente * columnas + indice] = hueco;

                //Requisito: es la nave es crucero, y choca contra un muro, desaparece todos los bichos de su fila o columna
                if (dev_matriz[filaActual * columnas + indice] == letraR) {
                    curandState localState = state[filaActual * columnas + indice]; // Obtener el estado de curand local para este hilo
                    ...
                    if (queDestruyo == 0) { //destruyo la fila
                        for (short j = 0; j < columnas; ++j) {
                            if (dev_matriz[filaActual * columnas + j] != letraBloque && dev_matriz[filaActual * columnas + j] != letraNave) {
                                dev_matriz[filaActual * columnas + j] = hueco;
                            }
                        }
                    }
                    else { //destruyo la columna
                        for (short i = 0; i < filas - 1; ++i) {
                            if (dev_matriz[i * columnas + indice] != letraNave && dev_matriz[i * columnas + indice] != letraBloque) {
                                dev_matriz[i * columnas + indice] = hueco;
                            }
                        }
                    }
                }
                ...
                dev_matriz[filaSiguiente * columnas + indice] = dev_matriz[filaActual * columnas + indice];
            }
...
__global__ void reconvertir_nube(char* matriz, short* d_filas, short* d_columnas) {
    ...
    // Verifica si la celda actual contiene letraN
    if (matriz[indice] == letraA) {


        if (fila > 0 && fila < filas - 1 && columna > 0 && columna < columnas - 1) {

            // Memoria compartida para la tesela
            __shared__ char matriz_shared[TILE][TILE];

            // Calcula el índice de la esquina superior izquierda de la tesela
            int indice_matriz = (fila - TILE / 2) * columnas + (columna - TILE / 2);

            // Carga la tesela en la matriz compartida
            for (int i = 0; i < TILE; i++) {
                for (int j = 0; j < TILE; j++) {
                    int indice_actual = indice_matriz + i * columnas + j;

                    // Comprobamos si estamos en el rango
                    if (indice_actual >= 0 && indice_actual < filas * columnas) {
                        matriz_shared[i][j] = matriz[indice_actual];
                    }
                    else {
                        matriz_shared[i][j] = hueco; // Fuera de los límites de la matriz global
                    }
                    __syncthreads();
                }
            }
            ...
            // Verificar si todas las celdas adyacentes son 'A'
            if (matriz_shared[0][1] == letraA && // Arriba
                matriz_shared[2][1] == letraA && // Abajo
                matriz_shared[1][0] == letraA && // Izquierda
                matriz_shared[1][2] == letraA)   // Derecha
            {
                adyacentes = true;
            }
        }
    ...
    // Si todas las celdas adyacentes son 'A', transforma la celda actual y las adyacentes
    if (adyacentes) {
        // adyacentes = false;
        if (matriz[indice] == letraA) matriz[indice] = letraN; // Transforma la celda actual en 'C'
        if (matriz[(fila - 1) * columnas + columna] == letraA) matriz[(fila - 1) * columnas + columna] = ' '; // Arriba
        if (matriz[(fila + 1) * columnas + columna] == letraA) matriz[(fila + 1) * columnas + columna] = ' '; // Abajo
        if (matriz[fila * columnas + (columna - 1)] == letraA) matriz[fila * columnas + (columna - 1)] = ' '; // Izquierda
        if (matriz[fila * columnas + (columna + 1)] == letraA) matriz[fila * columnas + (columna + 1)] = ' '; // Derecha
    }
}
...
__global__ void reconvertir_cefalopodo(char* matriz, short* d_filas, short* d_columnas) {
    ...
    if (matriz[indice] == letraN) {
        ...
        if (fila > 0 && fila < filas - 1 && columna > 0 && columna < columnas - 1) {
            ...
            // Verificar si todas las celdas adyacentes son letraA
            if (matriz_shared[0][1] == letraA && // Arriba
                matriz_shared[2][1] == letraA && // Abajo
                matriz_shared[1][0] == letraA && // Izquierda
                matriz_shared[1][2] == letraA)   // Derecha
    ...
    if (adyacentes) {
        // adyacentes = false;
        if (matriz[indice] == letraN) matriz[indice] = letraC; // Transforma la celda actual en letraC
        if (matriz[(fila - 1) * columnas + columna] == letraA) matriz[(fila - 1) * columnas + columna] = ' '; // Arriba
        if (matriz[(fila + 1) * columnas + columna] == letraA) matriz[(fila + 1) * columnas + columna] = ' '; // Abajo
        if (matriz[fila * columnas + (columna - 1)] == letraA) matriz[fila * columnas + (columna - 1)] = ' '; // Izquierda
        if (matriz[fila * columnas + (columna + 1)] == letraA) matriz[fila * columnas + (columna + 1)] = ' '; // Derecha
...
void mostrarDatosNvidia() {

    //recuperamos las propiedades de nuestra tarjeta de video:
    int deviceID = 0;
    ...
    int maxHilosPorBloque = deviceProp.maxThreadsPerBlock;

    printf("\n\n***************************************************\n");
    printf("DEVICE %d: %s\n", deviceID, deviceProp.name);
    printf("***************************************************\n");
    printf("Número máximo de hilos por bloque				\t:%d\n", maxHilosPorBloque);
    printf("Número máximo de hilos por Multiprocesador			\t:%d\n", deviceProp.maxThreadsPerMultiProcessor);
    printf("Dimensiones máximas para organizar los hilos en bloques (x,y,z) \t:(%d, %d, %d)\n", deviceProp.maxThreadsDim[0], deviceProp.maxThreadsDim[1], deviceProp.maxThreadsDim[2]);
    printf("Dimensiones máximas para organizar los bloques en el grid (x,y,z) \t:(%d, %d, %d)\n", deviceProp.maxGridSize[0], deviceProp.maxGridSize[1], deviceProp.maxGridSize[2]);
    printf("Memoria global máxima disponible (en GB)			\t:%zu\n", deviceProp.totalGlobalMem / 1048576000);
    printf("Memoria compartida (shared) disponible por bloque (bytes)	\t:%zu\n", deviceProp.sharedMemPerBlock);
    printf("Numero máximo de registros disponible por bloque		\t:%zu\n", deviceProp.regsPerBlock);
    printf("***************************************************\n");

    if (columnas > maxHilosPorBloque) {
        printf("\nEl número de columnas supera la capacidad de hilos permitida por la GPU");
        exit(0);
    }
...
int main(int argc, char* argv[]) {
    ...
    inicializarMatriz << <filas - 1, columnas >> > (d_matriz, d_state, d_posicionNave, d_filas, d_columnas);
    ...
    while (true) { //Este while true no le gustó a Lorena pero se lo explicamos
        ...
        reconvertir_cefalopodo << < filas - 1, columnas >> > (d_matriz, d_filas, d_columnas);
        reconvertir_nube << < filas - 1, columnas >> > (d_matriz, d_filas, d_columnas);
        reconvertir_comandante << < filas - 1, columnas >> > (d_matriz, d_state, d_filas, d_columnas);
        descensoYDesintegracion << <filas - 1, columnas >> > (d_matriz, d_state, d_filas, d_columnas);
        comprobarSuelo << < filas - 1, columnas >> > (d_matriz, d_state, d_posicionNave, d_vidas, d_puntos, d_filas, d_columnas);
        crearAlienigenas << <filas - 1, columnas >> > (d_matriz, d_state, d_filas, d_columnas);
