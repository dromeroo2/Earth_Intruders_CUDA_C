...
/* Voy a llamar al kernel "descensoYDesintegracion", y lo pongo como "global" para que sea accesible tanto
    para la GPU como para la CPU */
__global__ void onda_expansiva(char* dev_matriz, char nave, int posicion) {
    //short indice = threadIdx.x;  // aquí almaceno el índice del hilo dentro del bloque 
    //short filaActual = blockIdx.x; //
...
__global__ void reconvertir_nube(char* matriz, short* d_filas, short* d_columnas) {
    ...
    if (matriz[indice] == letraA) {
        // Verifica si todas las celdas adyacentes son letraA
            adyacentes = (fila > 0 && matriz[(fila - 1) * columnas + columna] == letraA) && // Arriba
            (fila < filas - 1 && matriz[(fila + 1) * columnas + columna] == letraA) && // Abajo
            (columna > 0 && matriz[fila * columnas + (columna - 1)] == letraA) && // Izquierda
            (columna < columnas - 1 && matriz[fila * columnas + (columna + 1)] == letraA); // Derecha
...
// Si todas las celdas adyacentes son letraA, transforma la celda actual y las adyacentes
        if (adyacentes) {
           ...
           if (matriz[(fila - 1) * columnas + columna] ==letraA) matriz[(fila - 1) * columnas + columna] = hueco; // Arriba
           if (matriz[(fila + 1) * columnas + columna] == letraA) matriz[(fila + 1) * columnas + columna] = hueco; // Abajo
...
// Si todas las celdas adyacentes son letraA, transforma la celda actual y las adyacentes
        if (adyacentes) {
            ...
            matriz[(fila - 1) * columnas + columna] = hueco; // Arriba
            matriz[(fila + 1) * columnas + columna] = hueco; // Abajo
